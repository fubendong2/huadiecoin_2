...
// Copyright (c) 2013 The huadiecoin developers
...
uint256 hashGenesisBlock("0x20eb93669c3df5900c7ce476ad740ddf1f3b0cf440178b2de01fe78f7d1cc3e5");
static const unsigned int timeGenesisBlock = 1395559046;
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 2); // joy joy
...
const string strMessageMagic = "huadiecoin Signed Message:\n";
...
static const int64 nTargetSpacing = 90; // 15 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	int nActualTimespanLong = (pindexLast->GetBlockTime() - pindexFirst->GetBlockTime())/4;
        int nActualTimespanAvg = (nActualTimespan + nActualTimespanLong)/2; // Average between short and long windows
        nActualTimespan = nActualTimespanAvg + 3*nTargetTimespan;
        nActualTimespan /= 4;

        printf("RETARGET: nActualTimespanLong = %d, nActualTimeSpanAvg = %d, nActualTimespan (with funk) = %"PRI64d"\n", nActualTimespanLong, nActualTimespanAvg, nActualTimespan);
	int nActualTimespanMax = nTargetTimespan*98/77; // add crack
        int nActualTimespanMin = nTargetTimespan*77/98;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x20eb93669c3df5900c7ce476ad740ddf1f3b0cf440178b2de01fe78f7d1cc3e5");
    }
...
*/
        ...
        // Genesis block
        const char* pszTimestamp = "huadiecoin baidu.com";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = nGenesisBlockRewardCoin;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0497ca50463741a59ae311cfbbd5285079d96a824d871000ada1e0c75b18e62b5c846281c3c2f1cffb8b8790bc031c9d533b5aa5ae94241959c00ef14ed4f03aae") << OP_CHECKSIG;
        CBlock block;
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0xb645dbd00c0713b3f9f3665c6d7bc020d3a620c9ac17382ff9c5d7ee7e4d2f60"));
...
unsigned char pchMessageStart[4] = { 0xdd, 0x03, 0xa5, 0xfe };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("huadiecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("huadiecoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("huadiecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("huadiecoin-miner");
    ...
    try { loop {
        ...
        printf("Running huadiecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("huadiecoinMiner terminated\n");
        throw;
    }
}
